...
static AlgorithmType CURRENT_ALGO = PABFD;  // Change this to switch algorithms
...
static const double   PABFD_U_LOW   = 0.20;   // consolidation source threshold
static const double   PABFD_U_HIGH  = 0.85;   // consolidation destination ceiling
static const double   PABFD_GPU_PENALTY = 500.0; // ΔP penalty for mismatched GPU
static const unsigned PABFD_CONSOLIDATE_INTERVAL = 150; // periodic-check ticks
...
static void PABFD_Consolidate(vector<MachineId_t>& machines, vector<VMId_t>& vms) {
    ...
    // (lightly-loaded candidates examined first).
    ...
    sort(by_util.begin(), by_util.end());
...
static void PABFD_DrainPending(vector<MachineId_t>& machines, vector<VMId_t>& vms) {
    for (auto& [cpu_type, bucket] : pabfd_pending) {
        ...
        // Sort descending by memory (BFD) then ascending by SLA urgency
        // so large, urgent tasks are placed before smaller, relaxed ones.
        stable_sort(bucket.begin(), bucket.end(), [](TaskId_t a, TaskId_t b) {
            unsigned ma = GetTaskMemory(a), mb = GetTaskMemory(b);
            if (ma != mb) return ma > mb;   // larger memory first (Decreasing)
            // tie-break: stricter SLA first
            SLAType_t sa = RequiredSLA(a), sb = RequiredSLA(b);
            int pa = (sa==SLA0)?0:(sa==SLA1)?1:(sa==SLA2)?2:3;
            int pb = (sb==SLA0)?0:(sb==SLA1)?1:(sb==SLA2)?2:3;
            return pa < pb;
...
void PABFD_PeriodicCheck(Time_t now,
    ...
    // Retry pending tasks in BFD order.
...
void Scheduler::MigrationComplete(Time_t time, VMId_t vm_id) {
    ...
    else if (CURRENT_ALGO == PABFD)         PABFD_MigrationComplete(vm_id, machines, vms);
}

void Scheduler::Shutdown(Time_t time) {
    if (CURRENT_ALGO == PMAPPER) { PMapper_Shutdown(vms); return; }
    if (CURRENT_ALGO == PABFD)   { PABFD_Shutdown(vms);  return; }
    for (VMId_t vm : vms) {
        if (CURRENT_ALGO == CUSTOM_GREEDY) {
...
void SLAWarning(Time_t time, TaskId_t task_id) {
    ...
    if (CURRENT_ALGO == CUSTOM_GREEDY) {
        ...
        // SLA warning: immediately wake ALL matching sleeping machines
        // and re-raise the task's priority so it jumps the pending queue.
...
void StateChangeComplete(Time_t time, MachineId_t machine_id) {
    if (CURRENT_ALGO == CUSTOM_GREEDY) {
        ...
        if (mi.s_state == S0) {
            // Restore peak performance on all cores.
            for (unsigned c = 0; c < mi.num_cpus; c++)
                Machine_SetCorePerformance(machine_id, c, P0);
            pabfd_retry_pending = true;
        }
